...
export default function AiFinal() {
  ...
  const stepLogic = (currentStep, userInput) => {
    ...
    switch (currentStep) {
      case 1:
        addMessage({ text: 'What kind of artist are you looking for?', options: defaultSuggestions.roles });
      ...
      case 2:
        if (userInput === 'Others') {
          addMessage({ text: "Please type the role you're looking for:" });
          setSuggestions([]);
          setStep(2.5);
        } else {
          data.roles = [userInput];
          setCollected(data);
          addMessage({ text: 'What genres fit your project?', options: defaultSuggestions.genres });
      ...
      case 2.5:
        ...
        addMessage({ text: 'We found these roles based on your input:', options: roleGen });
      ...
      case 3:
        if (userInput === 'Others') {
          addMessage({ text: "Please type the genre you're looking for:" });
          setSuggestions([]);
          setStep(3.5);
        } else {
          data.genres = [userInput];
          setCollected(data);
          addMessage({ text: 'Pick some vibe/mood tags for your collaborator:', options: defaultSuggestions.vibeTags });
      ...
      case 3.5:
        ...
        addMessage({ text: 'We found these genres based on your input:', options: genreGen });
      ...
      case 4:
        if (userInput === 'Others') {
          addMessage({ text: "Type the vibe/mood you're looking for:" });
          setSuggestions([]);
          setStep(4.5);
        } else {
          data.vibeTags = [userInput];
          setCollected(data);
          addMessage({ text: 'What language should they speak or sing in?', options: ['English', 'Hindi', 'Bengali', 'Others'] });
      ...
      case 4.5:
        ...
        addMessage({ text: 'We found these vibe tags based on your input:', options: vibeGen });
      ...
      case 5:
        if (userInput === 'Others') {
          addMessage({ text: 'Type the language:' });
          setSuggestions([]);
          setStep(5.5);
        } else {
          data.language = [userInput];
          setCollected(data);
          addMessage({ text: 'Where would you prefer your collaborator to be from?' });
          setSuggestions([]);
          setStep(6);
        }
        break;
      ...
      case 6:
        ...
        setCollected(data);
        addMessage({ text: 'Do you prefer Male, Female, or Any gender?', options: ['Male', 'Female', 'Any'] });
        setSuggestions(['Male', 'Female', 'Any']);
        setStep(7);
        break;

      case 7:
        data.genderPreference = userInput;
        setCollected(data);
        addMessage({ text: "Thanks for your info! Here's what you selected:" });
        addMessage({ text: JSON.stringify(data, null, 2) });
        addMessage({ text: 'Is everything correct?', options: ['Yes', 'No'] });
      ...
      case 9:
        if (userInput.toLowerCase() === 'yes') {
          addMessage({ text: 'Great! Searching for artist matches...' });
          setSuggestions([]);
        } else {
          setCollected({});
          addMessage({ text: 'Let’s start over. What kind of artist are you looking for?', options: defaultSuggestions.roles });
          setSuggestions(defaultSuggestions.roles);
          setStep(2);
        }
        break;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center z-0">
        <img src={bgGif} alt="background" className="max-w-full max-h-full object-contain" />
      </div>
